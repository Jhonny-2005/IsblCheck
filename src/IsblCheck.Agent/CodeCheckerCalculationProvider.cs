...
namespace IsblCheck.Agent
{
  internal class CodeCheckerCalculationProvider
  {
    ...
    private IEnumerable<IDocument> ParseCommonReports()
    {
      ...
      {
        if (string.IsNullOrEmpty(commonReport.CalculationText))
          continue;
    ...
    private IEnumerable<IDocument> ParseDialogs()
    {
      foreach (var dialog in this.context.Dialogs)
      {
        ...
        {
          if (action.ExecutionHandler != null || string.IsNullOrEmpty(action.CalculationText))
            continue;
        ...
        {
          if (string.IsNullOrEmpty(method.CalculationText))
            continue;
        ...
        {
          if (string.IsNullOrEmpty(@event.CalculationText))
            continue;
    ...
    private IEnumerable<IDocument> ParseFunctions()
    {
      ...
      {
        if (string.IsNullOrEmpty(function.CalculationText))
          continue;
    ...
    private IEnumerable<IDocument> ParseIntegratedReports()
    {
      ...
      {
        if (string.IsNullOrEmpty(integratedReport.CalculationText))
          continue;
    ...
    private IEnumerable<IDocument> ParseScripts()
    {
      ...
      {
        if (string.IsNullOrEmpty(script.CalculationText))
          continue;
    ...
    private IEnumerable<IDocument> ParseRouteBlocks()
    {
        ...
        .Where(b => b.WorkflowBlock != null))
      {
        ...
        {
          if (string.IsNullOrEmpty(action.CalculationText))
            continue;
