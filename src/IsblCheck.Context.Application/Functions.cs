...
namespace IsblCheck.Context.Application
{
  ...
  internal interface Functions
  {
    ...
    // Directum 5.8: Методы документа
    object AddTimestampToSignature(object Signature);
    object AddValidationDataToSignature(object Signature, bool OcspResponsesRequired);
    object AddArchiveTimestamp(object Signature);
    object AddTimestamp();
    object GetAccessibleLifeCycleStages();
    object SaveShadowCopiesToFile(int HistoryRecordID, string FileName);
    object DeleteShadowCopies(int VersionNumber);
    object SaveLifeCycleAsImage(string FileName, int ImageType);
    object ExportInExtendedFormat(int VersionNumber, string FileName, bool NeedLock, bool NeedCompress, int SignaturesType);
    object ImportInExtendedFormat(int VersionNumber, string FileName, bool NeedUnlock, string EditorCode);

    // Directum 5.8: Методы задач
    object CreateRouteStep();
    object GetTaskTree(bool UseCache);
    object CanParticipantsReadAttachments(bool IsForFamilyTask);

    // Directum 5.8: Методы заданий
    object PerformWithResult(string PerformResult);

    // Directum 5.8: Методы процессов
    object ExecuteSendHandler(object ProcessMessage, object Params);
    object ExecuteReceiveHandler(object ProcessMessage, object Params);
    object CreateProcessMessage(string MessageName);
    object LockWithTimeout(int Milliseconds);
    object CreateParam(int DataType);

    // Directum 5.8: Методы фабрик задач
    object CreateSubTaskToJob();
    object CreateSubTaskToTask();

    // Directum 5.8: Методы поисков
    object DeleteIndexData();
    object LoadAll();
    object UpdateIndexData();
    object GetContentKind();

    // Directum 5.8: Методы почты (IMailFactory, IMailServer)
    object MailFactory();
    object MailGetServer(string ServerType);
    object MailCreateMessage();
    object MailCreateParamList();
    object MailLogon(object Options);
    object MailLogoff();
    object MailSendMessage(object Message, object Options);
    object MailSearchInbox(object Criteria);
    object MailDeleteMessage(object Message);
    object MailMarkMessageRead(object Message);
    object MailMarkMessageUnread(object Message);

    // Directum 5.8: Методы серверных событий (IServerEventFactory)
    object ServerEventGetObjectByName(string Name);
    object ServerEventStartByName(string Name);
    object ServerEventStartByNameAt(string Name, DateTime StartDate);
    object ServerEventStart();
    object ServerEventStartAt(DateTime StartDate);

    // Directum 5.8: Методы глобальных ИД (IGlobalIDFactory)
    object GlobalIDCreateGlobalID(int LocalID, int ComponentType, string ComponentName, bool IsDefault, string AdditionalInfo);
    object GlobalIDFindGlobalIDs(int LocalID, int ComponentType, string ComponentName);
    object GlobalIDFindLocalIDs(string GlobalID);
    object GlobalIDGetObjectByGlobalID(string GlobalID);
    object GlobalIDObjectInfoByGlobalID(string GlobalID);
    object GlobalIDSaveGlobalID(string GlobalID, int LocalID, int ComponentType, string ComponentName, bool IsDefault, string AdditionalInfo);

    // Directum 5.8: Методы дерева задач
    object GetTreeRootNodes(bool UseCache);

    // Directum 5.8: Методы вложений
    object DeleteAttachments(object ObjectInfoList, string ErrorMessage);
    object Find(object AttachmentInfo);
    object Add(object AttachmentInfo);
    object Delete(object AttachmentInfo);
  }
}
