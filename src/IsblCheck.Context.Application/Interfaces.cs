...
namespace IsblCheck.Context.Application.Interfaces
{
  ...
  internal interface ILoginPoint
  {
    string HostName { get; }
    string HostAddress { get; }
  ...
  internal interface IMailFactory
  {
    int FreezeTimeOutInSeconds { get; set; }
    IMailServer GetServer(string ServerType);
    IMessage CreateMessage();
    IList CreateParamList();
  }

  internal interface IMailServer
  {
    void Logon(IList Options);
    void Logoff();
    void SendMessage(IMessage Message, IList Options);
    IList SearchInbox(IList Criteria);
    void DeleteMessage(IMessage Message);
    void MarkMessageRead(IMessage Message);
    void MarkMessageUnread(IMessage Message);
  }
