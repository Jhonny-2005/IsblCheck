...
{
  internal class SQLInjectionDirectConcatRule : AbstractRule
  {
    private const string Code = "S003";

    private static readonly Lazy<IRuleInfo> info = new Lazy<IRuleInfo>(() =>
      new RuleInfo(typeof(SQLInjectionDirectConcatRule).Name, "Правило обнаружения SQL-инъекций через конкатенацию переменных в запросах CSQL/CSQL/SQL."), true);

    public static IRuleInfo Info => info.Value;

    private static readonly HashSet<string> SQLFunctions = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
      "SQL", "CSQL", "SelectSQL", "CSelectSQL"
    };
    ...
    private class SQLInjectionListener : IsblBaseListener
    {
      ...
      public override void EnterFunction(IsblParser.FunctionContext context)
      {
        ...
        {
          if (HasDirectConcatWithVariable(expr))
          {
      ...
      private static bool HasDirectConcatWithVariable(IsblParser.ExpressionContext expr)
      {
    ...
    public override void Apply(IReport report, IDocument document, IContext context)
    {
      ...
      {
        report.AddError(Code, $"Критическая SQL-инъекция: конкатенация строк в запросе {call.identifier().GetText()}(). Используйте параметризованные запросы.", document, call.identifier().GetTextPosition());
      }
    }
  }
}
